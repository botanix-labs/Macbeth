...
3. Request "Pegout Transaction" endpoint creates a contract interaction transaction calling into the burn function of the genesis contract. Botanix consensus will listen for the burn topic emission and only generate a pegout if the user has enough funds to cover the pegout amount + L1 on-chain fees.
