...
The purpose of this document is to outline how the botanix protocol is going to maintain a its list of federation members. And how that list can be updated via a voting mechanism. There is a related goal of using the same list to acquired the aggregated public key used in a FROST signing group.
...
This is a major design constraint and should be thought about carefully.
...
As such, [Clique POA consensus](https://eips.ethereum.org/EIPS/eip-225#standardized-proof-of-authority) maintains the list of authorized signers fully contained in the block headers. Clique accomplishes this without making any changes to existing block headers.
...
* Follow up q: Should the initial set of signers get specified in chain spec config?
...
### How can we softfork staking requirements
