...
use merkle::get_wallet_state_commitment;
use pegout_scheduler::pegout_id::PegoutId;
use pegout_scheduler::PegoutRequest;
use rand::thread_rng;
use rpc::FILE_DESCRIPTOR_SET;
use shutdown::{stop_signal, StopHandle};
use signer::error::SigningRound1Error;
use thiserror::Error;
use tokio::sync::{oneshot, Mutex};
use tonic::{codegen::CompressionEncoding, metadata::BinaryMetadataKey, transport::Server};
use util::{get_available_utxos, get_pegin_confirmation_depth, parse_eth_address};
use wallet::{
    ...
    BitcoindClient: RpcApi + Send + Sync + 'static,
{
    ...
    async fn get_signing_status(
        ...
        let signing_session_id =
            util::parse_signing_session_id(&req.signing_session_id).to_status()?;
    ...
    async fn new_round1_signing_package(
        ...
        let signing_session_id =
            util::parse_signing_session_id(&req.signing_session_id).to_status()?;
        let frost_id = util::deserialize_frost_peer_id(req.identifier).to_status()?;
    ...
    async fn new_round1_dkg_package(
        ...
        let req = req.into_inner();
        let frost_id =
            crate::util::deserialize_frost_peer_id(req.identifier.clone()).to_status()?;
        let dkg_round1 =
...
mod tests {
    ...
    use crate::test_utils::{
        self, create_random_pegout_id, create_tx, random_p2wpkh_script, trusted_dealer_setup,
        MockBitcoind,
    };
    ...
    async fn setup() -> App<MockBitcoind> {
        ...
        let app = App::new(config, bitcoind_client, telemetry).expect("btc server");
    ...
    async fn round1_dkg_should_work_if_missing_key_package() {
        ...
        let frost_id = util::deserialize_frost_peer_id(inner.identifier).unwrap();
        assert_eq!(frost_id, frost_id!(1));
        let payload = inner.payload;
        let round1_dkg_pkg = frost::keys::dkg::round1::Package::deserialize(&payload).unwrap();
    ...
    async fn round2_dkg_fails_when_missing_round1_secret() {
        ...
        assert_eq!(res.message(), "missing round1 dkg package");
    }
