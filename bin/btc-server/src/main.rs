...
async fn stop_signal(grpc_stop_tx: Option<StopHandle>) {
...
impl App {
    ...
    pub async fn serve_async(&self, config: GrpcConfig) -> Result<StopHandle, Error> {
        ...
        let app = Self::new(self.config.clone())?;

        let mut btc_server = rpc::BtcServerServer::new(app)
        ...
        health_reporter
            .set_serving::<rpc::BtcServerServer<App>>()
            .await;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // init config
    let config = TomlConfig::new(args.config)
    .await
    .context("Failed to load config")?;

    // setup the grpc server
    let btc_server = App::new(config)?;

    // run grpc server in the background
    let grpc_stop_tx = match btc_server.serve_async(config.api.grpc.clone()).await {
        Ok(s) => {
            info!(
    ...
    tokio::spawn(stop_signal(grpc_stop_tx));
