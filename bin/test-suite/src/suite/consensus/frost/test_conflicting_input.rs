...
pub async fn test_conflicting_input(
    ...
    // 3) The network will reject the broadcast psbt if the replacement fee isn't high enough.  We are not trying to replace the tx
    //    so this is an acceptable failure case as well: "insufficient fee, rejecting replacement"
