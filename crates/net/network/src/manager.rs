...
pub struct NetworkManager<C> {
    ...
    /// Receiver half of the command channel set up between this type and the [`NetworkHandle`]
    from_frost_protol_events: UnboundedReceiverStream<FrostProtocolEvent>,
    /// Receiver half of the command channel set up between this type and the [`NetworkHandle`]
    ...
    C: BlockNumReader,
{
    ...
    pub async fn new(config: NetworkConfig<C>) -> Result<Self, NetworkError> {
        ...
        // add frost protocol
        let (from_frost_protol_events, from_frost_peers_messages) =
    ...
    C: BlockReader + Unpin,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        loop {
            match this.from_frost_protol_events.poll_next_unpin(cx) {
                Poll::Pending => break,
                Poll::Ready(None) => {
                    // This is only possible if the channel was deliberately closed since we always
                    // have an instance of `NetworkHandle`
