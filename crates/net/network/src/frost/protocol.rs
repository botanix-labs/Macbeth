        ...
        loop {
            ...
            if let Poll::Ready(Some(cmd)) = this.commands.poll_next_unpin(cx) {
                return match cmd {
                    ...
                    FrostPeerCommand::PingMessage { msg, response } => {
                    ...
                    FrostPeerCommand::PeerMessage(response) => {
                        ...
                        match response_type {
                            EventResponseType::DkgRound1 => {
                                info!(">>>>>>>>> SENDING ROUND 1 DKG DATA = {:?}", data);
                            ...
                            EventResponseType::DkgRound2 => {
                                info!(">>>>>>>>> SENDING ROUND 2 DKG DATA = {:?}", data);
                                let req = Request::new(identifier, data);
                                Poll::Ready(Some(
            ...
            match msg.message {
                ...
                FrostProtoMessageKind::PingMessage(_peer_id, authority_index) => {
                    info!(
                        ">>>>>>>>> RECEIVED PING FROM PEER WITH HIS AUTH INDEX = {}. SENDING PONG WITH MY AUTH INDEX",
                        authority_index
                    );
                ...
                FrostProtoMessageKind::PongMessage(peer_id, authority_index) => {
                    info!(
                        ">>>>>>>>> RECEIVED PONG FROM PEER WITH AUTH INDEX {}. CONFIRMING RECEIVED",
                        authority_index
                    );
                ...
                FrostProtoMessageKind::Round1Dkg(data) => {
                    ...
                    info!(">>>>>>>>> RECEIVED DKG 1 PACKAGE FROM PEER. -- FORWARDED.");
                }
                FrostProtoMessageKind::Round2Dkg(data) => {
                    ...
                    info!(">>>>>>>>> RECEIVED DKG 2 PACKAGE FROM PEER. -- FORWARDED.");
                }
            }
