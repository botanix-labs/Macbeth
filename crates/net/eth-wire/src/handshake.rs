...
use reth_eth_wire_types::{DisconnectReason, EthMessage, EthVersion, ProtocolMessage, Status};
    ...
    EthStreamError: From<E> + From<<S as Sink<Bytes>>::Error>,
{
    ...
    pub async fn eth_handshake(
        ...
        let status = unified_status;

        // Send our status message
        let status_msg =
            alloy_rlp::encode(ProtocolMessage::from(EthMessage::Status(unified_status))).into();
