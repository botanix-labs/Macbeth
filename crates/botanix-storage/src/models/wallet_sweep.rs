...
impl Compress for WalletSweepSession {
    ...
    fn compress_to_buf<B: bytes::BufMut + AsMut<[u8]>>(self, buf: &mut B) {
        ...
        // Write coordinator signature with length prefix
...
impl Decompress for WalletSweepSession {
    fn decompress<B: AsRef<[u8]>>(
        ...
        // Read coordinator signature (optional for backward compatibility)
        let coordinator_signature = if buf.remaining() >= 4 {
            ...
            if buf.remaining() >= signature_len {
                ...
                // Invalid signature length, default to empty
                vec![]
            }
        } else {
            // No signature data present (backward compatibility)
            vec![]
        };
...
mod tests {
    ...
    fn create_test_session() -> WalletSweepSession {
        ...
        WalletSweepSession { 
            ...
            fee_rate_sat_vb, 
            created_at,
            coordinator_signature: vec![0x01, 0x02, 0x03], // Test signature
        }
    }
    ...
    fn test_compress_decompress_empty_psbt() {
        ...
        let session =
            WalletSweepSession { 
                ...
                fee_rate_sat_vb, 
                created_at,
                coordinator_signature: vec![], // Empty signature for zero fee rate test
            };
    ...
    fn test_compress_decompress_large_psbt() {
        ...
        let session =
            WalletSweepSession { 
                ...
                fee_rate_sat_vb, 
                created_at,
                coordinator_signature: vec![0xFF; 64], // Large signature for max test
            };
    ...
    fn test_compress_decompress_different_networks() {
        ...
        for (network, address_str) in networks.iter().zip(addresses.iter()) {
            ...
            let session = WalletSweepSession {
                ...
                fee_rate_sat_vb,
                created_at,
                coordinator_signature: vec![0x42; 32], // Test signature for network tests
            };
