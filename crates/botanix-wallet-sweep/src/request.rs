...
impl WalletSweepRequest {
    ...
    fn determine_coordinator_id(
        ...
        let coordinator_key_data =
            fs::read_to_string(coordinator_key_path).wrap_err_with(|| {
                format!("Failed to read coordinator key from {:?}", coordinator_key_path)
            })?;

        let sanitized_key =
            coordinator_key_data.chars().filter(|c| c.is_ascii_hexdigit()).collect::<String>();
        ...
        for (index, member) in federation_config.federation_member_public_key.iter().enumerate() {
            let member_public_key =
                secp256k1::PublicKey::from_str(&member.key).wrap_err_with(|| {
                    format!("Invalid public key in federation config: {}", member.key)
                })?;
    ...
    pub fn build_with_federation_config(
        ...
        let coordinator_id =
            Self::determine_coordinator_id(coordinator_key_path, federation_config)?;
...
impl TryInto<WalletSweepSession> for WalletSweepRequest {
    ...
    fn try_into(self) -> Result<WalletSweepSession, Self::Error> {
        let bitcoin_network: Network =
            self.destination_network.parse().wrap_err("Invalid destination network")?;
