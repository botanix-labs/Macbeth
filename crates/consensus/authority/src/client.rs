...
impl AuthorityClient {
    ...
    async fn fetch_headers(&self, request: HeadersRequest) -> Vec<Header> {
        trace!(target: "consensus::authority", ?request, "received headers request");
        ...
        let mut block: BlockHashOrNumber = match start {
            ...
            BlockHashOrNumber::Number(num) => {
                if let Some(hash) = storage.block_hash(num) {
                    ...
                    warn!(target: "consensus::authority", num, "no matching block found");
                    return headers
                }
            }
        };
        ...
        //TODO(armins) Read in votes into storage
        trace!(target: "consensus::authority", ?headers, "returning headers");

        headers
    }

    async fn fetch_bodies(&self, hashes: Vec<B256>) -> Vec<BlockBody> {
        trace!(target: "consensus::authority", ?hashes, "received bodies request");
        let storage = self.storage.read().await;
        ...
        trace!(target: "consensus::authority", ?bodies, "returning bodies");

        bodies
    }
}

impl HeadersClient for AuthorityClient {
...
impl BodiesClient for AuthorityClient {
...
impl DownloadClient for AuthorityClient {
