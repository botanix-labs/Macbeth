...
impl DataParser {
    ...
    /// # Arguments
    ///
    /// * `serialization_type` - A `SerializationType` enum specifying the desired serialization
    ///   format.
    ///
    /// # Returns
    ///
    ...
    pub async fn compress(&self, data: &[u8]) -> Result<Vec<u8>, Error> {
        let decompressed_data = match &self.compression_strategy {
            Some(strategy) => strategy.compress(data).await?,
            None => data.to_vec(),
        };
    ...
    /// # Arguments
    ///
    /// * `data` - A byte slice (`&[u8]`) representing the serialized (and optionally compressed)
    ///   data.
    ///
    /// # Returns
    ///
    ...
    pub async fn decompress(&self, data: &[u8]) -> Result<Vec<u8>, Error> {
        let data = match &self.compression_strategy {
            Some(strategy) => strategy.decompress(data).await?,
            None => data.to_vec(),
        };
