...
use reth_chainspec::{ChainSpec, EthereumHardforks, BOTANIX_TESTNET};
...
pub fn validate_block_pre_execution(
    ...
    if chain_spec.is_shanghai_active_at_timestamp(block.timestamp) && chain_spec.chain.id() != BOTANIX_TESTNET.chain.id()
    {
        let withdrawals =
