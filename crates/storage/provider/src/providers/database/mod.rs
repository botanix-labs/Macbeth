...
impl<DB: Database> SnapshotReader for ProviderFactory<DB> {
    ...
    fn get_snapshot_id_by_block_height(
        ...
        self.provider()?.get_snapshot_id_by_block_height(block_id)
    }

    fn get_chunk_block_height(&self, chunk_id: ChunkId) -> ProviderResult<Option<BlockNumber>> {
        self.provider()?.get_chunk_block_height(chunk_id)
    }
...
impl<DB: Database> SnapshotWriter for ProviderFactory<DB> {
    ...
    fn create_new_snapshot(
        ...
        self.provider_rw()?.create_new_snapshot(block_id, block_hash, app_hash)
    }
